...
def write_html(jobs, path=HTML_FILE):
    """Write a self-contained, email-friendly HTML digest of today's matches.
    Uses inline styles so it renders in email clients. When jobs is empty the
    page carries a clear 'no matches' marker the routine can detect."""
    ...
    for j in jobs:
        ...
        rows.append(f"""
      <tr>
        <td style="padding:12px 14px;border-bottom:1px solid #e6e8ec;vertical-align:top;">
          <div style="font-weight:600;color:#16263F;font-size:15px;">{esc(j['title'])}</div>
          <div style="color:#5b6470;font-size:13px;margin-top:2px;">{esc(j['company'])} &middot; {esc(j['location'])}</div>
          <div style="color:#7a828c;font-size:12px;margin-top:4px;">{esc(j['why'])}</div>
        </td>
        <td style="padding:12px 14px;border-bottom:1px solid #e6e8ec;vertical-align:top;text-align:center;white-space:nowrap;">
          <span style="display:inline-block;background:#eaf0ff;color:#1a4fd6;font-weight:700;font-size:14px;padding:4px 10px;border-radius:12px;">{pct}%</span>
        </td>
        <td style="padding:12px 14px;border-bottom:1px solid #e6e8ec;vertical-align:top;white-space:nowrap;">
          <a href="{esc(j['url'])}" style="color:#1a4fd6;font-weight:600;text-decoration:none;font-size:14px;">Open &rarr;</a>
        </td>
      </tr>""")

    if jobs:
        body = f"""
    <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;background:#ffffff;border:1px solid #e6e8ec;border-radius:8px;overflow:hidden;">
      <thead>
        <tr style="background:#16263F;">
          <th align="left" style="padding:10px 14px;color:#ffffff;font-size:12px;letter-spacing:.04em;text-transform:uppercase;">Role</th>
          <th align="center" style="padding:10px 14px;color:#ffffff;font-size:12px;letter-spacing:.04em;text-transform:uppercase;">Match</th>
          <th align="left" style="padding:10px 14px;color:#ffffff;font-size:12px;letter-spacing:.04em;text-transform:uppercase;">Apply</th>
        </tr>
      </thead>
      <tbody>{''.join(rows)}
      </tbody>
    </table>"""
        intro = f"{len(jobs)} matched Product Manager role{'s' if len(jobs) != 1 else ''} today, scored &ge; {MIN_SCORE_PCT}% against your profile."
    else:
        body = """
    <div data-no-jobs="true" style="padding:24px;background:#ffffff;border:1px solid #e6e8ec;border-radius:8px;color:#5b6470;font-size:15px;">
      No strong matches found today — the agent ran successfully but nothing cleared the match threshold.
    </div>"""
        intro = "No strong matches found today."
...
<title>Daily PM jobs — {esc(today)}</title>
</head>
<body style="margin:0;padding:0;background:#f4f5f7;">
  <div style="max-width:720px;margin:0 auto;padding:24px 16px;font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#1f2933;">
    <h1 style="margin:0 0 4px;font-size:20px;color:#16263F;">Daily PM jobs</h1>
    <div style="color:#7a828c;font-size:13px;margin-bottom:16px;">{esc(today)}</div>
    <p style="font-size:14px;color:#3b4453;margin:0 0 16px;">{intro}</p>
    {body}
    <p style="color:#9aa1ab;font-size:11px;margin-top:20px;">
      Cybersecurity / Cloud Product Manager roles in Israel &middot; generated by your daily job agent.
    </p>
  </div>
</body>
</html>"""
